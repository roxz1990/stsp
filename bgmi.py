...
BOT_TOKEN = "8150550366:AAFbeqWeGLA_QpthrANjwgHA9TQxxcOIUc4"
    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @Roxz_gaming"
        else:
            response = "📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @Roxz_gaming"
    else:
        response = f"📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @Roxz_gaming"
...
def start_command(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "WELCOME TO 🥇PREMIUM🥇 USER--> @Roxz_gaming.", reply_markup=main_menu())
